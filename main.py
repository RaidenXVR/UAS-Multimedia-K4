...
tab1, tab2, tab3 = st.tabs(["Image Compression", "Video Compression", "Audio Compression"])

# Image Compression
with tab1:
    st.header("Compress an Image")
    img_file = st.file_uploader("Upload an image", type=["jpg", "jpeg", "png"], key="img")
    quality = st.slider("Compression Strength (JPEG Quality)", 1, 100, 75, key="img_quality")
    ...
    if img_file:
        ...
        st.download_button("Download Compressed Image", compressed_data, file_name="compressed.jpg", mime="image/jpeg")


# Video Compression
with tab2:
    st.header("Compress a Video")
    video_file = st.file_uploader("Upload a video", type=["mp4", "mov", "avi"], key="vid")
    bitrate = st.slider("Compression Bitrate (kbps)", 300, 5000, 1000, key="vid_bitrate")
    ...
    if video_file:
        ...
        clip.write_videofile(output_path, bitrate=f"{bitrate}k", audio_codec='aac', logger=None)
        ...
        st.download_button("Download Compressed Video", compressed_data, file_name="compressed_video.mp4", mime="video/mp4")
...
with tab3:
    ...
    audio_file = st.file_uploader("Upload an audio", type=["mp3", "wav", "ogg", "m4a"], key="aud")
    ...
    if audio_file:
        ...
        st.download_button("Download Compressed Audio", compressed_audio, file_name="compressed_audio.mp3", mime="audio/mpeg")
